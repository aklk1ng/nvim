...
end, { desc = 'Add buffer diagnostics to the location list and set the location list title' })
map('n', '<C-S-l>', function()
  vim.diagnostic.setloclist({
    title = vim.fn.expand('%'),
    severity = vim.diagnostic.severity.ERROR,
  })
end, { desc = 'Add buffer ERROR diagnostics to the location list and set the location list title' })
...
map('i', '<C-l>', function()
  vim.cmd('normal! zz')
end, { desc = 'Redraw, current line at center of window and leave the cursor in the same column' })
...
map('c', '<A-f>', '<C-Right>')
map('c', '<C-e>', '<End>')
map('c', '%c', "<C-R>=expand('%:p:~:h')<CR>", { desc = 'Cwd' })
map('c', '%p', "<C-R>=expand('%:p:~')<CR>", { desc = 'Full path' })
-- Terminal mappings
map('t', '<A-Esc>', '<C-\\><C-n>')
...
map('n', '<leader>g', _G._cmd('Gitsigns diffthis vertical=true split=rightbelow'))
map('n', ']c', function()
  if vim.wo.diff then
...
map('n', '<leader>fc', _G._cmd('FzfLua command_history'))
map('n', '<leader>fo', _G._cmd('FzfLua oldfiles'))
map('n', '<leader>fs', _G._cmd('FzfLua lsp_document_symbols'))
map('n', '<leader>fS', _G._cmd('FzfLua lsp_workspace_symbols'))
map('n', '<leader>fh', _G._cmd('FzfLua helptags'))
map('n', '<leader>fk', _G._cmd('FzfLua keymaps'))
map('n', '<leader>ft', _G._cmd('FzfLua tabs'))
map('n', '<leader>fm', _G._cmd('FzfLua manpages'))
map('n', '<leader>fd', _G._cmd('FzfLua lsp_workspace_diagnostics'))
map('n', '<leader>fl', _G._cmd('FzfLua loclist_stack'))
...
map('n', '<leader>fq', _G._cmd('FzfLua quickfix_stack'))

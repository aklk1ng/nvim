...
local function display(opts)
  ...
  if (opts.pos == 'sp') or (opts.pos == 'vsp') then
    local pos_type = pos_data[opts.pos]
    opts.size = opts.size and opts.size or config.sizes[opts.pos]
    local new_size = vim.o[pos_type.area] * opts.size
...
---Hide all created terminal windows.
_G.Terms.hideall = function()
  for _, info in pairs(vim.g.terms) do
    if vim.api.nvim_buf_is_valid(info.buf) and vim.fn.bufwinid(info.buf) ~= -1 then
...
---Resume all terminal windows.
_G.Terms.resumeall = function()
  for _, term in pairs(vim.g.terms) do
